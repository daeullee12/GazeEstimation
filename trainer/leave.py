...
def main(config):
    ...
    # Ensure transformer layers are frozen
    if config.train.freeze_transformer:
        net.freeze_transformer_layers()
    ...
    # Ensure transformer layers are frozen
    if config.train.freeze_transformer:
        transformer_params = sum(p.numel() for name, p in net.named_parameters() if 'encoder' in name and p.requires_grad)
        print(f"Trainable transformer parameters: {transformer_params}")
        assert transformer_params == 0, "Transformer layers are not frozen!"

    # Pretrain
    pretrain = config.train.pretrain
    if pretrain.enable:
        # Load transformer weights from the pre-trained ResNet18+transformers model
        map_location = f"cuda:{pretrain.device}" if 'device' in pretrain else 'cpu'
        transformer_checkpoint = torch.load(pretrain.path, map_location=map_location)
        transformer_state_dict = {k: v for k, v in transformer_checkpoint.items() if 'encoder' in k or 'cls_token' in k or 'pos_embedding' in k}
        net.load_transformer_weights(transformer_state_dict)
        ...
        if config.train.backbone == 'efficientnet-b0':
            ...
            net.load_state_dict(transformer_checkpoint, strict=False)
